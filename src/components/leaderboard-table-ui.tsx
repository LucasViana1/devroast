...
interface LeaderboardTableUIProps {
  data: {
...
export function LeaderboardTableUI({ data }: LeaderboardTableUIProps) {
  ...
  return (
    <>
          ...
          <TableBody>
            {data.entries.map((entry) => (
              <TableRow key={entry.id}>
                <TableCell
                  className={entry.rank <= 3 ? "text-accent-amber" : "text-text-secondary"}
                >
                  {entry.rank}
                </TableCell>
      ...
      <p className="mt-4 text-center font-secondary text-xs text-text-tertiary">
        showing {data.entries.length} of {data.totalRoasts.toLocaleString()} roasts
      </p>
    </>
  );
}
